...
void entry(unsigned long magic, unsigned long addr) {
    ...
    if (CHECK_FLAG(mbi->flags, 3)) {
        ...
        while (mod_count < mbi->mods_count) {
            ...
            if(mod_count == 0){ file_system_init((uint32_t)mod->mod_start); } //initialize our file system
    ...
    memset(terminals, 0, sizeof(terminal_t)*3);
